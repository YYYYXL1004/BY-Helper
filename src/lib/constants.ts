...
export const applicationStatusBadge: Record<ApplicationStatus, string> = {
  ...
  WAITING_RESULT: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300',
...
export function normalizeApplicationStatus(status?: string | null): ApplicationStatus {
  if (status === 'SCREEN_PASSED') return 'SHORTLISTED'
  return status && status in applicationStatusLabels ? status as ApplicationStatus : 'WATCHING'
}
